-- Пишет из stg.kafka_moex_trades_local в dds.moex_candles_1d (Distributed), агрегируя сделки
-- в дневные OHLCV-свечи. Независима от ods.mv_moex_trades_local — ClickHouse допускает
-- несколько независимых MV на одной Kafka-таблице, каждая получает свою копию консюмленных
-- блоков
DROP VIEW IF EXISTS dds.mv_moex_candles_1d_local ON CLUSTER sharded SYNC;
CREATE MATERIALIZED VIEW IF NOT EXISTS dds.mv_moex_candles_1d_local ON CLUSTER sharded
TO dds.moex_candles_1d AS
    SELECT
        sec_id,
        board_id,
        toStartOfInterval(trade_dttm, INTERVAL 1 DAY) AS candle_dttm,
