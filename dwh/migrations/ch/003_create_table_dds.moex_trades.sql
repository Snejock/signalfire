DROP TABLE IF EXISTS dds.moex_trades_local ON CLUSTER sharded SYNC;
CREATE TABLE IF NOT EXISTS dds.moex_trades_local ON CLUSTER sharded
(
    _loaded_dttm        DateTime('UTC') DEFAULT now(),
    _source_system      LowCardinality(String),
    trade_no            UInt64,
    trade_dttm          DateTime('UTC'),
...
-- Distributed-таблица поверх dds.moex_trades_local — точка входа для записи/чтения
DROP TABLE IF EXISTS dds.moex_trades ON CLUSTER sharded SYNC;
CREATE TABLE IF NOT EXISTS dds.moex_trades ON CLUSTER sharded
AS dds.moex_trades_local
ENGINE = Distributed(sharded, dds, moex_trades_local, cityHash64(sec_id))
;