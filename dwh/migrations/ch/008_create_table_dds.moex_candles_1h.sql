-- Слой DDS: часовые OHLCV-свечи по сделкам MOEX, агрегируются из Kafka в реальном времени.
-- Хранятся бессрочно
DROP TABLE IF EXISTS dds.moex_candles_1h_local ON CLUSTER sharded SYNC;
CREATE TABLE IF NOT EXISTS dds.moex_candles_1h_local ON CLUSTER sharded
(
...
-- Distributed-таблица поверх dds.moex_candles_1h_local — точка входа для записи/чтения
DROP TABLE IF EXISTS dds.moex_candles_1h ON CLUSTER sharded SYNC;
CREATE TABLE IF NOT EXISTS dds.moex_candles_1h ON CLUSTER sharded
AS dds.moex_candles_1h_local
ENGINE = Distributed(sharded, dds, moex_candles_1h_local, cityHash64(sec_id))
;
